...
#Updated Sokoban model with domain-specific techniques
...
def compute_deadlock_map(puzzle):
    height = len(puzzle)
    width = len(puzzle[0])
    deadlock_map = [[False for _ in range(width)] for _ in range(height)]
    ...
    def no_goal_in_direction(y, x, dy, dx):
        while in_bounds(y, x) and not is_wall(y, x):
            if is_goal(y, x):
                return False
            y += dy
            x += dx
    ...
    for y in range(1, height - 1):
        for x in range(1, width - 1):
            ...
            #corner
            if (
                (is_wall(y - 1, x) and is_wall(y, x - 1)) or
                (is_wall(y - 1, x) and is_wall(y, x + 1)) or
                (is_wall(y + 1, x) and is_wall(y, x - 1)) or
                (is_wall(y + 1, x) and is_wall(y, x + 1))
            ):
                deadlock_map[y][x] = True
                continue

            #edge
            if y == 1:  # Top edge
                if no_goal_in_direction(y, x, 0, -1) and no_goal_in_direction(y, x, 0, 1):
                    deadlock_map[y][x] = True
            elif y == height - 2:  # Bottom edge
                if no_goal_in_direction(y, x, 0, -1) and no_goal_in_direction(y, x, 0, 1):
                    deadlock_map[y][x] = True
            elif x == 1:  # Left edge
                if no_goal_in_direction(y, x, -1, 0) and no_goal_in_direction(y, x, 1, 0):
                    deadlock_map[y][x] = True
            elif x == width - 2:  # Right edge
                if no_goal_in_direction(y, x, -1, 0) and no_goal_in_direction(y, x, 1, 0):
                    deadlock_map[y][x] = True
...
def solve_sokoban(puzzle_lines):
    ...
    deadlocks = compute_deadlock_map(puzzle)
    ...
    deadlock_map = model.add_element_table(deadlocks)
    ...
    push_up = dp.Transition(
        ...
        preconditions=[
            ...
            ~(deadlock_map[(player_pos - 2 * width) // width, (player_pos - 2 * width) % width] == 1)
        ],
    )
    ...
    push_left = dp.Transition(
        ...
        preconditions=[
            ...
            ~(deadlock_map[(player_pos - 2) // width, (player_pos - 2) % width] == 1)
        ],
    )
    ...
    solver = dp.CABS(model, time_limit=10, keep_all_layers=True)
    #solver = dp.CAASDy(model, time_limit=60)
...
if __name__ == "__main__":
    ...
    plt.plot(puzzle_numbers, solve_times, marker='o', linestyle='-', color='teal')
    plt.title('Solve Time per Puzzle')
